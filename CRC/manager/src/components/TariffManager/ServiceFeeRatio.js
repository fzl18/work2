...
export default class ServiceFeeRatio extends Component {
state = {
    loading:false,
    pagination:{
  ...
  loadListData = (params) => {
    ...
    const options ={
        method: 'POST',
        url: API_URL.serive.queryPlatformServiceFeeRatio,
        data: {
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        title: '会员类型',
        width:100,
        render:(text,record)=> record.memberType.memberTypeName=='INSIDE_ASSISTANT' ? '临床协调员（内部）':'临床协调员（外部）'
      },
      {
        title: '平台服务费比例',
        width:160,

        ...
        return(            
          ...
          >
            <FormItem {...formItemLayout} label="会员类型">
              ...
              <FormItem {...formItemLayout} label="订单补贴比例">
              {getFieldDecorator('ratio',{
                  rules: [{
                    required: true, message: '请输入',
                  }],
                })(
                  <InputNumber min={0} precision={0} max={100} style={{width:150}} placeholder='请输入'/>
              )} %
              </FormItem>
              <FormItem {...formItemLayout} label="红包奖励">
              {getFieldDecorator('voucherCount',{
                  rules: [{
                    required: true, message: '请输入',
                  }],
                })(
                  <InputNumber min={0} precision={0} style={{width:150}} placeholder='（等额服务红包）'/>
              )} 小时
              </FormItem>
...
export default class SericeMemberProfit extends Component {
state = {
    loading:false,
    pagination:{
  ...
  loadListData = (params) => {
    ...
    const options ={
        method: 'POST',
        url: API_URL.serive.queryServiceRuleMemberProfit,
        data: {
  ...
  loadMemberLevel=()=>{
    const options ={
        ...
        doneResult: data => {
            if (!data.error) {
                this.setState({
                  memberLevelType: data.datas || data.data,
                });
            } else {
                Modal.error({ title: data.error });
            }
  ...
  renderSearchForm() {
    const { selectedRows, searchFormValues,memberLevel,memberType, memberLevelType } = this.state;
  ...
  save = (params) => {
    ...
    const options ={
        method: 'POST',
        url: isEdit ? API_URL.serive.modifyServiceRuleMemberProfit :  API_URL.serive.addServiceRuleMemberProfit,
        data: {
            ...params,
  ...
  del = (id) => {
    const options ={
        method: 'POST',
        url: API_URL.serive.deleteServiceRuleMemberProfit,
        data: {
            offset: 1,
            limit: 1,
            ruleMemberProfitId:id,
  ...
  render() {
    const {loading, listData, detail, selectedRows, isEdit, selectedRowKeys, totalCallNo, modalVisible, pagination,memberLevelType,memberType } = this.state;
    const columns = [
      {
        title: '序号',
        dataIndex: 'index',
    ...
    return (
      <div>
            ...
            />
            <Modal
                title={isEdit ? '修改服务会员权益':'添加服务会员权益'}

          ...
          >
            ...
            >
              {getFieldDecorator('serviceParameterName', {
                rules: [{
                  required: true, message: '请输入授权工作类型',                  
                },{
                  max:255, message:'限制长度255个字符',
                }],
              })(
                <Input placeholder="请输入授权工作类型" />
              )}
            </FormItem>
...
state = {
  ...
  loadListData = (params={}) => {
    ...
    command.api({
      url:API_URL.serive.queryJobType,
      pagination,
      params,
    },data => {
      if(!data.error){
  ...
  del=(params={})=>{
    command.api({
      url:API_URL.serive.deleteJobType,
      params,
    },data =>{
      if(!data.error){
  ...
  save=(params={})=>{
    ...
    command.api({
      url: isEdit ? API_URL.serive.modifyJobType : API_URL.serive.addJobType,
      params,
    },data =>{
      if(!data.error){
        notification['success']({
  ...
  handleSubmit = (e) => {
    e.preventDefault();
    const {isEdit,editId}=this.state
    const serviceParameterId = isEdit ? editId : null
    this.formboxref.validateFieldsAndScroll((err, values) => {
      if (!err) {
        this.save({...values,serviceParameterId})
      }
    });
  }
  ...
  render() {
    ...
    return (
      <div>
            ...
            />
            <Modal
                title={isEdit ? '修改授权工作类型':'添加授权工作类型'}

...
class FormBox extends React.Component {
    ...
    componentDidMount(){
      ...
      this.setState({isResult,memberBehaviorType,order},()=>{setFieldsValue({
        order
      })})
    }
    ...
    render(){
        ...
        const memberBehaviorTypeOptions = memberBehaviorType && memberBehaviorType.map((v) => <Option key={v.memberBehaviorTypeId} value={`${v.memberBehaviorTypeId}`}>{ v.memberBehaviorTypeName == '完成支付/订单结算收款' ? '完成支付' : v.memberBehaviorTypeName}</Option>)
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        title: '行为',
        width:130,
        render:(text,record)=> record.memberBehaviorType && record.memberBehaviorType.memberBehaviorTypeName == '完成支付/订单结算收款' ? '完成支付' : record.memberBehaviorType.memberBehaviorTypeName    
      },      
      {
        title: '行为结果',
        width:160,
    ...
    return (
      <div>
            ...
            />
            <Modal
                title={isEdit ? '修改下单会员积分制度':'添加下单会员积分制度'}

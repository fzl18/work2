  ...
  $.fn.data = function(key, value) {
    if (typeof value === 'undefined') {
      ...
      if (this[0] && this[0].getAttribute) {
        var dataKey = this[0].getAttribute('datas-' + key);
    ...
    var Template7 = function (template) {
      ...
      function getCompileVar(name, ctx) {
        ...
        for (var i = 0; i < parts.length; i++) {
          ...
          if (part.indexOf('@') === 0) {
            if (i > 0) {
              variable += '[(datas && datas.' + part.replace('@', '') + ')]';
            }
            else {
              variable = '(datas && datas.' + name.replace('@', '') + ')';
            }
          }
          else {
            if (isFinite(part)) {
      ...
      function compile(template, depth) {
        ...
        var ctx = 'ctx_' + depth;
        var resultString = '(function (' + ctx + ', datas) {\n';
        if (depth === 1) {
        ...
        for (i = 0; i < blocks.length; i++) {
          ...
          if (block.type === 'helper') {
            if (block.helperName in t.helpers) {
              ...
              resultString += 'r += ($.Template7.helpers.' + block.helperName + ').call(' + ctx + ', ' + (compiledArguments && (compiledArguments + ', ')) +'{hash:' + JSON.stringify(block.hash) + ', datas: datas || {}, fn: ' + getCompileFn(block, depth+1) + ', inverse: ' + getCompileInverse(block, depth+1) + ', root: ctx_1});';
            }
            else {
              if (block.contextName.length > 0) {
                ...
                resultString += 'r += ($.Template7.helpers.each).call(' + ctx + ', ' + variable + ', {hash:' + JSON.stringify(block.hash) + ', datas: datas || {}, fn: ' + getCompileFn(block, depth+1) + ', inverse: ' + getCompileInverse(block, depth+1) + ', root: ctx_1});';
                resultString += '}else {';
                resultString += 'r += ($.Template7.helpers.with).call(' + ctx + ', ' + variable + ', {hash:' + JSON.stringify(block.hash) + ', datas: datas || {}, fn: ' + getCompileFn(block, depth+1) + ', inverse: ' + getCompileInverse(block, depth+1) + ', root: ctx_1});';
                resultString += '}}';
              }
            }
          }
...
Input.prototype = {
    /**
     * should handle the inputEvent datas and trigger the callback
     * @virtual
     */
    handler: function() { },
...
/**
 * extend the datas with some usable properties like scale, rotate, velocity etc
...
+ function($) {
  ...
  $.modal = function(params) {
    ...
    var buttonsHtml = buttons.map(function(d, i) {
      return '<a class="weui_btn_dialog ' + (d.className || "") + '">' + d.text + '</a>';
    }).join("");
  ...
  //提示跳转页面去掉‘确认’
  $.alert_noBtn = function(text, title, callback) {
    if (typeof title === 'function') {
      callback = arguments[1];
      title = undefined;
    }
...
+ function($) {
  ...
  var Picker = function (params) {
      ...
      var defaults = {
          ...
          <div class="toolbar-inner">\
          <a href="javascript:;" class="picker-button-un close-picker" id="picker-button-un">取消</a>\
          </div>\
      ...
      p.initPickerCol = function (colElement, updateItems) {
          ...
          col.setValue = function (newValue, transition, valueCallbacks) {
              ...
              var newActiveIndex = col.wrapper.find('.picker-item[datas-picker-value="' + newValue + '"]').index();
          ...
          col.updateItems = function (activeIndex, translate, transition, valueCallbacks) {
              ...
              */
              ...
              if (valueCallbacks || typeof valueCallbacks === 'undefined') {
                  // Update values
                  col.value = selectedItem.attr('datas-picker-value');
          ...
          function handleClick(e) {
              ...
              $.cancelAnimationFrame(animationFrameId);
              /*jshint validthis:true */
              var value = $(this).attr('datas-picker-value');
              col.setValue(value);
          }
      ...
      p.columnHTML = function (col, onlyItems) {
          ...
          if (col.divider) {
              ...
              for (var j = 0; j < col.values.length; j++) {
                  columnItemsHTML += '<div class="picker-item" datas-picker-value="' + col.values[j] + '">' + (col.displayValues ? col.displayValues[j] : col.values[j]) + '</div>';
              }
...
+ function($) {
  ...
  Select.prototype.updateInputValue = function(values, titles) {
    ...
    this.$input.attr("value", t).attr("datas-values", v);
      ...
      '<div class="weui_cells weui_cells_radio">\
        ...
        <label class="weui_cell weui_check_label" for="weui-select-id-{{this.title}}">\
          ...
          <div class="weui_cell_ft">\
            <input type="radio" class="weui_check" name="weui-select" id="weui-select-id-{{this.title}}" value="{{this.value}}" {{#if this.checked}}checked="checked"{{/if}} datas-title="{{this.title}}">\
    ...
    checkboxTemplate:
      '<div class="weui_cells weui_cells_checkbox">\
        ...
        <label class="weui_cell weui_check_label" for="weui-select-id-{{this.title}}">\
          ...
          <div class="weui_cell_ft">\
            <input type="checkbox" class="weui_check" name="weui-select" id="weui-select-id-{{this.title}}" value="{{this.value}}" {{#if this.checked}}checked="checked"{{/if}} datas-title="{{this.title}}" >\
            <span class="weui_icon_checked"></span>\
          </div>\
        </label>\
        {{/items}}\
  ...
  var Calendar = function (params) {
      ...
      p.updateValue = function () {
          ...
          for (i = 0; i < p.value.length; i++) {
              ...
              p.wrapper.find('.picker-calendar-day[datas-date="' + valueDate.getFullYear() + '-' + valueDate.getMonth() + '-' + valueDate.getDate() + '"]').addClass('picker-calendar-day-selected');
          }
          if (p.params.onChange) {
      ...
      p.initCalendarEvents = function () {
          ...
          function handleDayClick(e) {
              ...
              var dateYear = day.attr('datas-year');
              var dateMonth = day.attr('datas-month');
              var dateDay = day.attr('datas-day');
      ...
      p.monthHTML = function (date, offset) {
          ...
          for (i = 1; i <= rows; i++) {
              ...
              for (j = 1; j <= cols; j++) {
                  ...
                  rowHTML += '<div datas-year="' + dayYear + '" datas-month="' + dayMonth + '" datas-day="' + dayNumber + '" class="picker-calendar-day' + (addClass) + '" datas-date="' + (dayYear + '-' + dayMonth + '-' + dayNumber) + '"><span>'+dayNumber+'</span></div>';
              }
              monthHTML += '<div class="picker-calendar-row">' + rowHTML + '</div>';
          }
          monthHTML = '<div class="picker-calendar-month" datas-year="' + year + '" datas-month="' + month + '">' + monthHTML + '</div>';
          return monthHTML;
      };
      p.animating = false;
      p.updateCurrentMonthYear = function (dir) {
          if (typeof dir === 'undefined') {
              p.currentMonth = parseInt(p.months.eq(1).attr('datas-month'), 10);
              p.currentYear = parseInt(p.months.eq(1).attr('datas-year'), 10);   
          }
          else {
              p.currentMonth = parseInt(p.months.eq(dir === 'next' ? (p.months.length - 1) : 0).attr('datas-month'), 10);
              p.currentYear = parseInt(p.months.eq(dir === 'next' ? (p.months.length - 1) : 0).attr('datas-year'), 10);
          }
      ...
      p.nextMonth = function (transition) {
          ...
          var nextMonth = parseInt(p.months.eq(p.months.length - 1).attr('datas-month'), 10);
          var nextYear = parseInt(p.months.eq(p.months.length - 1).attr('datas-year'), 10);
      ...
      p.prevMonth = function (transition) {
          ...
          var prevMonth = parseInt(p.months.eq(0).attr('datas-month'), 10);
          var prevYear = parseInt(p.months.eq(0).attr('datas-year'), 10);
...
+ function($) {
  ...
  /*新增需求begin 年月日*/
  var DatetimeNew = function(input, params) {
    this.input = $(input);
    this.params = params;
  ...
  $.fn.datetimePickerNew = function(params) {
    params = $.extend({}, defaults, params);
    return this.each(function() {
      if(!this) return;
      var $this = $(this);
      var datetime = $this.data("datetime");
      if(!datetime) $this.data("datetime", new DatetimeNew(this, params));
      return datetime;
    });
  };
  /*新增需求end*/
  ...
  /*新增需求begin 年*/
  var DatetimeYear = function(input, params) {
    this.input = $(input);
    this.params = params;
  ...
  $.fn.datetimePickerYear = function(params) {
    params = $.extend({}, defaults, params);
    return this.each(function() {
      if(!this) return;
      var $this = $(this);
      var datetime = $this.data("datetime");
      if(!datetime) $this.data("datetime", new DatetimeYear(this, params));
      return datetime;
    });
  };
  /*新增需求end*/
  ...
  /*新增需求begin 年月*/
    var DatetimeYearMonth = function(input, params) {
    this.input = $(input);
    this.params = params;
  ...
  $.fn.datetimePickerYearMonth = function(params) {
    params = $.extend({}, defaults, params);
    return this.each(function() {
      if(!this) return;
      var $this = $(this);
      var datetime = $this.data("datetime");
      if(!datetime) $this.data("datetime", new DatetimeYearMonth(this, params));
      return datetime;
    });
  };
  /*新增需求end*/
  ...
  /*新增需求begin 时分*/
  var DatetimeDividerMinutes = function(input, params) {
    this.input = $(input);
    this.params = params;
  ...
  DatetimeDividerMinutes.prototype = {
    getDays : function(max) {
      var days = [];
      for(var i=1; i<= (max||31);i++) {
        days.push(i < 10 ? "0"+i : i);
    ...
    formatValue : function(values, displayValues) {
      var params = this.params;
      return values[0] + params._dateSplit + values[1];
    },
    stringToArray: function(value) {
      var params = this.params;
      var tokens = value.split(params.dateTimeSplit);
      var date = tokens[0],
          time = tokens[1];
      return [].concat(date.split(params._dateSplit), time ? time.split(params.timeSplit) : []);
    },
    arrayToDate: function(arr) {
      var params = this.params;
      if(arr.length === 3) return new Date(arr.join(params._dateSplit));
      var date = new Date(arr.slice(0, 3).join(params._dateSplit));
      //注意这种格式 "2012-12-12 12:12" 在ios上是错误的，如果用 "2012-12-12T12:12" 是对的，但是这个是标准时区而不是东八区，所以这里分别设置
      date.setHours(arr[3]);
      date.setMinutes(arr[4]);
      return date;
    ...
    getConfig : function() {
      ...
      var config = {
        ...
        value: [this.formatNumber(today.getHours()), this.formatNumber(today.getMinutes())],
  ...
  $.fn.datetimePickerDividerMinutes = function(params) {
    params = $.extend({}, defaults, params);
    return this.each(function() {
      if(!this) return;
      var $this = $(this);
      var datetime = $this.data("datetime");
      if(!datetime) $this.data("datetime", new DatetimeDividerMinutes(this, params));
      return datetime;
    });
  };
  /*新增需求end*/
  ...
  defaults = $.fn.datetimePickerNew.prototype.defaults = {
    dateSplit: "-",
    _dateSplit:":",
    timeSplit: ":",
    dateTimeSplit: " ",

...
class WithdrawalsDetailInfo extends React.Component {
  state={
    dataSource,
  ...
  onRefresh = () => {
    setTimeout(() => {
      ...
      this.props.dispatch({ type: 'CustomerServiceCenter/question', payload: {} });
    }, 1000);
  };
          ...
          >
            <Item
              onClick={() => {
                ...
                this.props.history.push(`/Problem/${obj.questionTypeId}`);
              }}
              className={styles.Question}
              arrow="horizontal"
            >
              <Brief style={{ marginLeft: 5, position: 'relative', fontSize: 15, color: '#333e4d' }} >{obj.questionTypeName}</Brief>
            </Item>
          </SwipeAction>
        </List>
      );
            ...
            <div className={styles.AuthNoticeLess}>
              <ListView
                ...
                pullToRefresh={this.props.pullRefresh ? <PullToRefresh
                  refreshing={this.props.loading.effects['CustomerServiceCenter/question']}
...
export default connect(mapStateToProps)(withRouter(WithdrawalsDetailInfo));

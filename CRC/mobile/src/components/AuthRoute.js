  ...
  componentWillMount() {
    if (!window.configs.DEV) {
      if (!sessionStorage.role) {
        ...
        location.href = `https://open.weixin.qq.com/connect/oauth2/authorize?appid=${window.configs.APPID}&redirect_uri=${window.configs.JSP_URL}/oauth.do?url=${url}&response_type=code&scope=snsapi_userinfo&state=CRCCALL#wechat_redirect`;
      }
    }
  }
          ...
          if (routeAuth) {
            if (curAuth[routeAuth]) {
              if (curAuth[routeAuth].access) {
                ...
                const { currentApp = 'ORDER' } = sessionStorage;
                if (currentApp == 'ORDER') {
                  return (<Route
                  ...
                  />);
                } else if (currentApp == 'SERVICE') {
                  return (<Route
                    path={child.props.path}
                    render={
                            () => {
                              location.href = '/LoginForAssistant';
                            }
                        }
                  />);
                }

...
class Nav extends React.Component {
    ...
    getMenus = () => {
        let menuNode = 
            menuList.map((levelone,index) => {
                if(!levelone.children || levelone.show){
                    return <Menu.Item key={levelone.path}>
                        <a onClick={()=>{
                            this.gourl(levelone.path);
                        }}> { levelone.icon ? <i className={`anticon iconfont ${levelone.icon}`} /> : <Icon type='plus'/>}  <span >{levelone.name}</span></a></Menu.Item>
                    ...
                    {
                        levelone.children.map((leveltwo,i) => {
                            return <Menu.Item key={leveltwo.path}>
                                <a onClick={()=>{
                                    this.gourl(leveltwo.path);
                                }}>
                                {leveltwo.name}
                                </a>
                                </Menu.Item>;
    ...
    render() {        
        ...
        return (
            ...
            <nav className="nav-bar" style={!this.props.collapsed ?{overflowY:'auto',height:'85vh',width:"220px"} :null}>

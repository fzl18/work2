...
class FormBox extends React.Component {
    ...
    handleSelect=(v)=>{
      // console.log('handleSelect',this.state.sourceData,v.key)
      ...
      this.state.sourceData.map(data => {
        if(data.ydataAccountId == v.key){
          ...
          form.setFieldsValue({
            doctor:{name:data.userCompellation,id:data.ydataAccountId}
          });
        }
      })    
    }
    ...
    selectChange=(value)=>{
      ...
      if(!value.key){  
        ...
        this.setState({searchValue:value.key})
        if(sourceData){
    ...
    handleSubmit = (e) => {
        ...
        this.formboxref.validateFieldsAndScroll((err, values) => {          
          if (!err) {
            ...
            // if(!values.ydataAccountId || !values.doctor ||values.doctor && values.doctor.id != values.ydataAccountId ){ // || !values.doctor || values.doctor.id != values.ydataUuId){
      ...
      save = (params) => {
        ...
        const options ={
            ...
            data: {
                ...params,
                ydataAccountId: isEdit ? editId : params.ydataAccountId
            },
            dataType: 'json',
            doneResult: data => {
                if (!data.error) {

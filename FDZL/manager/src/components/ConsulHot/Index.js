    ...
    render(){
        ...
        return (
            <Form onSubmit={this.props.handleSearch} layout="inline">
                ...
                <Button icon="search" type="primary" htmlType="submit" style={{marginLeft:20}}>搜索</Button>
                <Link style={{marginLeft:20}} to='/consulhot/diy'><Button icon="plus" type="primary">自定义添加</Button></Link>
            </Form>
        );
    }
}
...
checkShowTable = () => {
    ...
    if((urlHash.startsWith("#/consulhot/detail"))||(urlHash.startsWith("#/consulhot/diy"))){
      this.setState({
        showStyle: "none",
      })
    }else{
  ...
  handleSearch = (e) => {
    ...
    this.searchFormRef.validateFields((err, fieldsValue) => {
      ...
      fieldsValue.msg = fieldsValue.msg && fieldsValue.msg.trim()
      fieldsValue.counselors = fieldsValue.counselors && fieldsValue.counselors.trim()
      fieldsValue.quizzers = fieldsValue.quizzers && fieldsValue.quizzers.trim()
      fieldsValue.solvers = fieldsValue.solvers && fieldsValue.solvers.trim()
      this.setState({
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text,record,index) => (
          moment(record.hotTime).format("YYYY-MM-DD HH:mm")
        )
      },      
      {
        title: '置顶',
      ...
      },
      {
        ...
        width:110,
        render: (text,record,index) => (
          record.topTime && moment(record.topTime).format("YYYY-MM-DD HH:mm")
        )
      },
      {
        title: '操作',

...
state = {
  ...
  checkShowTable = () => {
    ...
    if(urlHash.startsWith("#/education/detail/save")){
      this.setState({
        showStyle: "none",
      })
    }else{
  ...
  handleSearch = (e) => {
    ...
    this.searchFormRef.validateFields((err, fieldsValue) => {
      ...
      fieldsValue.popularScienceTitle = fieldsValue.popularScienceTitle && fieldsValue.popularScienceTitle.trim()
      this.setState({
    ...
    return (
        <Row gutter={2}>
            ...
            <Col md={2} sm={8} style={{textAlign:'right'}}>            
            {
                ...
                <Link to='/education/detail/save'><Button icon="plus" type="primary">添加</Button></Link>
            </Col>
        </Row>
    );
  }
    ...
    const columns = [
      ...
      },      
      {
        ...
        render: (text,record,index) => (
          <div>
            <Link to={`/education/detail/save/${record.id}`}>修改</Link>

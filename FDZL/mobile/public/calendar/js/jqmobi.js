...
document = window.document,
      ...
      var $jqm = function (toSelect, what) {
          ...
          let dom = this.selector(toSelect, what);
          if (!dom) {
              return this;
            }
...
key;
      ...
      $.jsonP = function (options) {
          let callbackName = `jsonp_callback${  ++_jsonPID}`;
          let abortTimeout = '',
              context;
...
error = false;
      ...
      function detectUA($, userAgent) {
          ...
          $.os.ios6 = !!($.os.ios &&  userAgent.match(/(OS)\s([6])/));
          $.os.playbook = !!userAgent.match(/PlayBook/);
          $.os.blackberry = !!($.os.playbook || userAgent.match(/BlackBerry/));
          $.os.blackberry10 = !!($.os.blackberry && userAgent.match(/Safari\/536/));
          $.os.chrome = !!userAgent.match(/Chrome/);
          $.os.opera = !!userAgent.match(/Opera Mobi/);
          $.os.fennec = !!userAgent.match(/fennec/i);
      ...
      function findHandlers(element, event, fn, selector) {
          ...
          return (handlers[jqmid(element)] || []).filter((handler) => {
      ...
      function add(element, events, fn, selector, getDelegate) {
          let id = jqmid(element),
              set = (handlers[id] || (handlers[id] = []));

          eachEvent(events, fn, (event, fn) => {
              let delegate = getDelegate && getDelegate(fn, event),
                  callback = delegate || fn;
              let proxyfn = function (event) {
                  let result = callback.apply(element, [event].concat(event.data));
                  if (result === false)
                      {event.preventDefault();}
                  return result;
                };
              let handler = $.extend(parse(event), {
                  fn,
                  proxy: proxyfn,
                  sel: selector,
                  del: delegate,
      ...
      $.fn.one = function (event, callback) {
          return this.each(function (i, element) {
              add(this, event, callback, null, (fn, type) => {
                  return function () {
                      let result = fn.apply(element, arguments);
...
match = $(e.target).closest(selector, element).get(0);
  ...
  if (kill && node.dispatchEvent) {
	            let e = $.Event('destroy', { bubbles: false });
	            node.dispatchEvent(e);
}
			// cleanup itself
  let id = jqmid(node);
  if (id && handlers[id]) {
		    	for (let key in handlers[id])
  ...
  cleanUpNode(node, kill);
      ...
      let cleanUpAsap = function (els, kill) {

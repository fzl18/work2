...
(function (root, factory) {
  ...
  var tpl = [
    '<div class="cal-wrap">', // id="{%scrollIntoId%}"
    '<h2 class="cal-backcolor"><i class="icon iconfont icon-fon10" onclick="prevMonth();"></i><span class="calendarDate">{%date%}</span><i class="icon iconfont icon-Previous" onclick="nextMonth();"></i></h2>',
    '<ul class="week-f"><li style="margin-top: 20px;" class="wk">周日</li><li style="margin-top: 20px;">周一</li><li style="margin-top: 20px;">周二</li><li style="margin-top: 20px;">周三</li><li style="margin-top: 20px;">周四</li><li style="margin-top: 20px;">周五</li><li class="wk" style="margin-top: 20px;">周六</li></ul>',
    '<ul class="day">',
    '{%day%}',
    '</ul>',
    '</div>',
  ...
  $.extend(calendar.prototype, {
    ...
    _getDayList: function (year, month) {
      ...
      for (var i = 0, len = tmpDayDataArr.length; i < len; i++) {
        ...
        if (roleName == 'PATIENT') {
          if (visitToDoList != '' && visitToDoList != null && visitToDoList != undefined) {
            ...
            for (var v = 0; v < visitToDoList.length; v++) {
              ...
              // if (new Date(util.formatDate(new Date()).replace(/\-/g, "/")) == new Date(util.formatDate(new Date(visitToDoDate.replace(/\-/g, "/"))))) {
              //   $("#patientTitle").html(util.formatDate(new Date(util.formatDate(new Date()).replace(/\-/g, "/"))) + ',祝您安康</br>按照医嘱，本日您可以提交随访资料');
              //   $("#patientVisit").show();
              // } else if (new Date(util.formatDate(new Date()).replace(/\-/g, "/")) >= new Date(util.formatDate(new Date(visitCanCreateBegin.replace(/\-/g, "/")))) && 
              // new Date(util.formatDate(new Date()).replace(/\-/g, "/")) <= new Date(util.formatDate(new Date(visitCanCreateEnd.replace(/\-/g, "/"))))) {
              //   $("#patientTitle").html(util.formatDate(new Date(util.formatDate(new Date()).replace(/\-/g, "/"))) + ',祝您安康</br>按照医嘱，本日您可以提交随访资料');
              //   $("#patientVisit").show();
              // } else {
              //   $("#patientTitle").html(util.formatDate(new Date(util.formatDate(new Date()).replace(/\-/g, "/"))) + ',祝您安康</br>本日没有医嘱待办事项');
              //   $("#patientVisit").hide();
              // }

              var classStyle = '';
              if (item.date == util.formatDate(new Date(visitToDoDate.replace(/\-/g, "/")))) {
                classStyle = 'crf1_class section_class ';
                foundPatient = true;
                break;
              } else {
                if (item.date == util.formatDate(new Date(visitCanCreateBegin.replace(/\-/g, "/")))) {
                  classStyle = 'createBegin_class ';
                  foundPatient = true;
                  break;
                } else if (item.date == util.formatDate(new Date(visitCanCreateEnd.replace(/\-/g, "/")))) {
                  classStyle = 'createEnd_class ';
                  foundPatient = true;
                  break;
                } else if (item.date > util.formatDate(new Date(visitCanCreateBegin.replace(/\-/g, "/"))) && item.date < util.formatDate(new Date(visitCanCreateEnd.replace(/\-/g, "/")))) {
                  classStyle = 'section_class ';
                  foundPatient = true;
                  break;
                }
              }
            ...
            if (foundPatient) {
              tmpDayTplArr.push(
                '<li class="' + classStyle + item.class + '" data-date="' + item.date + '">' +
                '<i>' + item.day + '</i>' +
                '</li>'
              );
            } else {
        ...
        } else if (roleName == 'DOCTOR' || roleName == 'ASSISTANT') {
          if (toDoDateList != '' && toDoDateList != null && toDoDateList != undefined) {
            var found = false;
            for (var k = 0; k < toDoDateList.length; k++) {
              var toDoDate = toDoDateList[k];
              if (item.date == util.formatDate(new Date(toDoDate.replace(/\-/g, "/")))) {
                found = true
                break;
              }
            }
            ...
            if (found) {
              // tmpDayTplArr.push(
              //   '<li class="crf1_class ${classStyle} ${item.class}" data-date="${item.date}">' +
